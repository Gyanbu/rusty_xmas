...
fn main() {
    ...
    let instructions: Vec<Vec<[u32; 2]>> = instructions
        .iter()
        .map(|(first, second)| {
            let first_parsed: Vec<_> = first
                .split(',')
                .map(|pos| pos.parse::<u32>().unwrap())
                .collect();
            let second_parsed: Vec<_> = second
                .split(',')
                .map(|pos| pos.parse::<u32>().unwrap())
                .collect();
            vec![
                [first_parsed[0], first_parsed[1]],
                [second_parsed[0], second_parsed[1]],
            ]
        })
        .collect();
    ...
    for instruction in &instructions {
        ...
        if start[0] == end[0] {
            ...
            let x_offset = if start[0] < end[0] { 1 } else { -1 };
            let y_offset = if start[1] < end[1] { 1 } else { -1 };
            for (i, _) in (start[0].min(end[0])..=start[0].max(end[0])).enumerate() {
                board[(start[0] as i32 + i as i32 * x_offset) as usize]
                    [(start[1] as i32 + i as i32 * y_offset) as usize] += 1;
            }
        }
    }
    let overlaps = board
